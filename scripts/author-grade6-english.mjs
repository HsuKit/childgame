...
function englishChoice(point, serial, difficulty, position) {
  ...
  if (point === '多段文本') return difficulty === 1
    ? choice(`Project: “${p.title}”. Text A gives a need; Text B gives a condition. What should the team do?`, p.summary, ['a topic unrelated to either text', 'only the colour of the page', 'a plan with no audience or evidence'], position, 'The answer combines the need and the condition.')
    : choice(`Read two short texts for “${p.title}”: ${sources(p)} What issue do both texts help the team understand?`, p.summary, ['a topic unrelated to either text', 'only the colour of the page', 'a plan with no audience or evidence'], position, 'The answer combines the key contribution of both texts.')
  ...
  if (point === '信息归纳') return difficulty === 1
    ? choice(`Project: “${p.title}”. Which summary keeps the main point and condition?`, p.summary, ['copy only the first number', 'delete the issue', 'combine unrelated actions'], position, `The summary represents both sources: ${p.summary}.`)
    : choice(`Project notes: ${sources(p)} Which summary keeps the main point without losing an important condition?`, p.summary, ['copy only the first number', 'keep one attractive detail and delete the issue', 'combine two unrelated actions'], position, `The summary represents both sources: ${p.summary}.`)
  if (point === '语境推断') return difficulty === 1
    ? choice(`Project: “${p.title}”. Which inference stays careful?`, p.inference, ['the same result is certain everywhere forever', 'no further checking can ever be useful', 'the project has no real purpose'], position, `The inference stays within the evidence: ${p.inference}.`)
    : choice(`From the two sources in “${p.title}”, what can we infer without going beyond the evidence?`, p.inference, ['the same result is certain everywhere forever', 'no further checking can ever be useful', 'the project has no real purpose'], position, `The inference follows both sources and stays within their evidence: ${p.inference}.`)
  ...
  if (point === '情境交际') return choice(`In “${p.title}”, a teammate may have missed a condition. Which reply is constructive?`, `Let’s check both texts and mark which detail supports each part.`, ['There is no need to read the sources.', 'Your idea is wrong because I said so.', 'Let’s delete every different opinion.'], position, 'The reply is polite and proposes an evidence-based way to resolve uncertainty.')
...
export function authorGrade6English() {
  ...
  for (const [type, difficulty, count] of distribution) for (let index = 0; index < count; index += 1) {
    ...
    questions.push({ id: `g6-english-authored-${String(serial + 1).padStart(3, '0')}`, subject: 'english', grade: 6, difficulty, type, knowledgePoint: point, skill: difficulty === 1 ? 'understand' : difficulty === 2 ? 'apply' : 'reason', tags: ['全国通用', difficulty > 1 ? 'cross-text reasoning' : 'real-task English'], content, reviewStatus: 'reviewed', version: 2 }); serial += 1
  }
  return questions
}
