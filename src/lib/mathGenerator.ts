...
const grade4: Gen[] = [
  ...
  () => { const price = rand(20, 80) * 10; const offPct = pick([20, 30, 25, 50]); const paid = Math.round(price * offPct / 100); const { options, answer } = opts(`${paid}元`, [`${price - offPct}元`, `${Math.round(price * (100 - offPct) / 100)}元`, `${price + offPct}元`]); return { stem: `商场打折，一件衣服原价${price}元，打${offPct}折后多少钱？`, options, answer, explanation: `${price}×${offPct}%=${paid}元` } },
...
// ===== GRADE 5 =====
const grade5: Gen[] = [
  () => { const price = rand(20, 80) * 10; const pct = pick([80, 85, 90, 70]); const paid = price * pct / 100; return { stem: `一双鞋原价${price}元，打${pct}折，要付多少？`, options: [`${paid}元`, `${price - pct}元`, `${price * (100 - pct) / 100}元`, `${price + pct}元`], answer: 0, explanation: `${price}×${pct}%=${paid}元` } },
