...
| ID | 日期 | 状态 | 决策 | 影响域 | 记录 |
| --- | --- | --- | --- | --- | --- |
| ADR-0001 | 2026-07-28 | accepted | 以 reviewed canonical JSON 作为题库唯一权威内容源 | 题库、发布、运行时读取 | [记录](./ADR-0001-canonical-question-json.md) |
...
ADR 是决策历史，不因结论变化而静默重写。若重大取舍发生变化，应新增 ADR，在新记录中说明它 supersede 的旧 ADR，并把旧记录状态改为 `superseded`；旧记录仍保留当时的背景、原因和影响。
