...
包含线上只读数据核查、余额 RPC 修复、check-in 愿望币补偿、幂等调用后的余额刷新和回归测试。不修改奖励数量、连续天数规则、数据库表结构或愿望商城交互。
...
- 以目标用户认证上下文调用线上 `get_wish_coin_balance`，复现 PostgreSQL `total_earned is ambiguous` 错误；`018_fix_wish_coin_balance.sql` 为 `returns table` 同名输出变量涉及的来源列增加限定，并重申 authenticated-only 执行权限。
...
沿用数据库 `(user_id, reason, reference_id)` 唯一约束和 RPC advisory lock 作为防重复边界，不做直接账本补写或历史批量修正。余额函数使用新增迁移替换，不改写已经应用的 `013`；客户端增加幂等重试。这样既修正所有用户的余额读取，也可恢复中断请求，并且不会绕过数据库对三科作答证据的校验。
...
- `node --test scripts/tests/wish-balance-migration.test.mjs`：1 项通过，并已先验证缺少迁移时失败。

## 风险与遗留

积分、check-in 与愿望账本仍不是单一数据库事务。客户端补偿覆盖正常加载和结算路径，但持续离线时只能等待下一次在线加载。当前没有真实 PostgreSQL 网络中断或并发集成测试；迁移部署后的远端 RPC 与页面验证记录在本次任务发布结果中。
